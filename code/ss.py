...
boxes_seg, masks_seg, cls_seg, probs_seg = predict_on_image(model_seg, image, conf=0.5)
for box_i, mask_i, cls_i, probs_i in zip(boxes_seg, masks_seg, cls_seg, probs_seg):
    ...
    if currentClass=='box1':
        ...
        # index=pd.DataFrame({'A': area,'B': range(len(area))})
        # index=list(index.sort_values('A', ascending=False)['B'].head(5))
        print(index)
#
#
#
